...
class Assistant(Agent):
    ...
    def __init__(
        ...
        *,
        ...
        protocol_instructions = _format_protocol_context(protocol_id, protocol_context)
        super().__init__(
    ...
    async def _publish_kol_result(self, result) -> None:
        """Publish a structured KOL Copilot result for UI cards/drawers."""
        if self._room is None:
            return
        try:
            payload = {
            ...
            logger.exception("Failed to publish kol_result data")

    @function_tool()
    async def answer_kol_question(self, context: RunContext, query: str) -> str:
        """Run the KOL Copilot workflow for Medical Affairs expert discovery.

        Call this for Phase 3 protocol questions, KOL discovery, investigator
        ranking, expert comparisons, compliant next actions, or MSL pre-call
        brief requests. It returns a concise voice summary and publishes the
        full structured result to the room as `kol_result`.

        Args:
            query: The user's protocol-aware KOL or Medical Affairs request.
        """
...
async def my_agent(ctx: JobContext):
    ...
    await session.generate_reply(
        instructions=(
            ...
            "KOL Copilot for Medical Affairs teams, say you are already scoped "
            f"to {_protocol_greeting_label(protocol_id, protocol_context)}, "
            "and invite them to ask a protocol-aware KOL discovery or MSL "
            "pre-call brief question."
        )
    )
